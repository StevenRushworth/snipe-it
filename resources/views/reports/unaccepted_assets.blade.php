                  ...
                  <tr @if($item['acceptance']->trashed()) style="text-decoration: line-through" @endif>
                    ...
                    <td>
                        @if(!$item['acceptance']->trashed())
                            @if ($item['acceptance']->assignedTo)<a href="{{ route('reports/unaccepted_assets_sent_reminder', ['acceptanceId' => $item['acceptance']->id]) }}" class="btn btn-sm bg-rpc-dark_green" data-tooltip="true">{{ trans('admin/reports/general.send_reminder') }}</a>@endif
